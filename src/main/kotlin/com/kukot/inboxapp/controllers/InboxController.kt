...
class InboxController(
    ...
    @GetMapping
    fun viewHomePage(@RequestParam("folder", required = false) folder: String?, model: Model, @AuthenticationPrincipal principal: OAuth2User?): String {
        val loginId = principal?.getAttribute<String>("login")
        if (loginId != null) {
            val inboxEmailList: List<EmailListItem> =
                emailListItemRepository.findAllByKey_IdAndKey_Label(loginId, folder ?: AppConstants.INBOX_FOLDER_LABEL)
